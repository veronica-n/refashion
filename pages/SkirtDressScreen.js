...
function SkirtDressScreen({ navigation }) {
    ...
    const subscriber = firestore()
      ...
      .onSnapshot(querySnapshot => {
        ...
        querySnapshot.forEach(documentSnapshot => {
          if (documentSnapshot.data().category == "Skirts & Dresses")
            skirtDress.push(documentSnapshot.data());
        });
        setSkirtDress(skirtDress);
      });
  ...
  return (
    <View style={{ flex: 1, alignItems: 'center', backgroundColor: '#F0EFEB' }}>
      <Text style={styles.title}>SKIRTS & DRESSES</Text>
